...
let valuesMatch(param: string, expected : int, tested : int) : Boolean = 
    if param.Equals("cats") || param.Equals("trees") then tested > expected
    else if param.Equals("pomeranians") || param.Equals("goldfish") then tested < expected
    else tested = expected

let auntsEqual(searched : Map<string, int>, potential: Map<string, int>) : Boolean = 
    let potentialWithoutId = Map.remove "id" potential
    (Map.filter (fun k v -> valuesMatch(k, searched.Item k, v)) potentialWithoutId).Count = potentialWithoutId.Count
