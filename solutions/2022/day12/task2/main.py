...
def read_input() -> tuple[list[list[int]], set[tuple[int, int]], tuple[int, int]]:
    data = []
    starts = set()
    end = None
...
def count_steps(g: dict[tuple[int, int], set[tuple[int, int]]], e: tuple[int, int], targets: set[tuple[int, int]]) -> int:
    to_visit = deque([(e, 0)])
    visited = set()
    awaiting = {e}
    while True:
...
board, starts, end = read_input()
graph = build_graph(board)
result = count_steps(graph, end, starts)
print(result)
