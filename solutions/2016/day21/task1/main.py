...
def transform(password, command):
    ...
    if match_rotate_pos is not None:
        ...
        rotation_length = (pos + (2 if pos >= 4 else 1)) % len(password)
        return password[-rotation_length:] + password[:-rotation_length]
    ...
    if match_move is not None:
        ...
        return password[:p2] + [letter] + password[p2:]
    return None


def scramble(password, commands):
    for c in commands:
...
result = ''.join(scramble(password, commands))
print(result)
